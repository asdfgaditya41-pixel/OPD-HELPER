...
Widget build(BuildContext context) {
    ...
    body: Container(
      ...
      child: StreamBuilder<Hospital>(
        ...
        builder: (context, snapshot) {
          ...
          final lastUpdated =
              hospitalVm.getTimeAgoFormatted(hospital.lastUpdated);
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _toolTile(
                      ...
                      subtitle: 'Update available bed counts',
                      color: const Color(0xFF66BB6A),
                      onTap: () {
                        _showBedManagement(context, hospital);
                      },
                    ),
                    _toolTile(
                      context: context,
  ...
  void _showBedManagement(BuildContext context, Hospital hospital) {
    final service = FirestoreService();
    int total = hospital.bedsTotal > 0 ? hospital.bedsTotal : hospital.bedsAvailable;
    int available = hospital.bedsAvailable;
    bool isSaving = false;
                  ...
                  Row(
                    children: [
                      Container(
                        padding: const EdgeInsets.all(10),
                        decoration: BoxDecoration(
                          color: const Color(0xFF66BB6A).withOpacity(0.16),
                          borderRadius: BorderRadius.circular(14),
                        ),
                        child: const Icon(
                          Icons.bed_rounded,
                          ...
                          : () async {
                              if (total < 0 || available < 0 || available > total) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Invalid bed counts.'),
                              ...
                              try {
                                await service.updateBedsModule(
                                  hospital.id,
                                  total,
                                  available,
                                );
                      ...
                      onPressed: isSaving
                          ...
                          : () async {
                              if (opd < 0 || emergency < 0) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Queue counts cannot be negative.'),
                                    backgroundColor: Color(0xFF122A34),
                                  ),
                                );
                                return;
